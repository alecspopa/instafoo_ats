  ...
  describe "creation" do
    context "with valid attributes" do
      ...
      it "creates a job_application event successfully" do
        expect { described_class.create(job_application: job_application, interview_date: interview_date) }.to \
          change { described_class.count }.by(1)

        last_record = described_class.last
        expect(last_record.type).to eq(described_class.name)
        expect(last_record.interview_date).to eq(interview_date.to_s)
      end
    end
    ...
    context "when another event for job application exists" do
      let(:job_application) { create(:job_application) }
      let(:interview_date) { Date.current }
      let!(:other_job_application_event) { create(:job_application_event_rejected, job_application: job_application) }
      ...
      it "does not create a new job_application event mapping" do
        expect { described_class.create(job_application: job_application, interview_date: interview_date) }.not_to \
    ...
    context "with invalid attributes" do
      ...
      context "when interview_date is not a date" do
        ...
        it "does not create a job_application event" do
          expect { described_class.create(job_application: job_application, interview_date: interview_date) }.not_to change { described_class.count }
        end
      end
    end
  end
